...
describe('Feature Controllers', () => {
  ...
  it('createAnnotation returns 201 with populated annotation', async () => {
    ...
    // Stub Annotation.create
    sandbox.stub(Annotation, 'create').resolves({ _id: annId });

    // Stub Activity.create
    sandbox.stub(Activity, 'create').resolves({});

    // Create a chainable populate stub
    const populatedAnnotation = { _id: annId, book: { title: 't' }, text: 'Note' };
    const populateStub = sinon.stub().resolves(populatedAnnotation);

    // Return an object with populate() function
    sandbox.stub(Annotation, 'findById').returns({ populate: populateStub });

    const req = { 
      user: { _id: mongoose.Types.ObjectId() }, 
      body: { bookId: mongoose.Types.ObjectId(), page: 3, text: 'Note' } 
    };
    const res = mockResponse();
    const next = mockNext();
    ...
    expect(res.status.calledWith(201)).to.be.true;
    expect(res.json.calledWith(populatedAnnotation)).to.be.true;
  });
